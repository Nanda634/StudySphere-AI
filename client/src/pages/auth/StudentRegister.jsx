...
export default function StudentRegister() {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    try {
      const data = await sendRegistrationOtp(form.name, form.email, form.password, "STUDENT");
  ...
  return (
    <div className="max-w-md mx-auto px-6 pt-20">
      <h1 className="font-display text-3xl mb-1">Create your student account</h1>
      <p className="text-ink2 mb-8">
        {step === "form" ? "Free, and takes about 30 seconds. We'll email you a code to verify it's really you." : "One more step."}
      </p>
      ...
      <p className="text-ink2 text-sm mt-4 text-center">
        Already have an account? <Link to="/student/login" className="text-glow hover:underline">Sign in</Link>
      </p>
    </div>
  );
}
